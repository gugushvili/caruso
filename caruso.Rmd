...
You can now play the caruso.wav file on your computer^[If you are a Mac user and have followed the directions in note 4, you can play the wave data via ```play(caruso_wave)```.]. For convenience, I've also linked it [here](../caruso.wav).
...
Finally, I'll create and save the wave file  of the de-noised signal. It's also available for download [here](../caruso_ebayes.wav).
...
A quick visual impression is that the method has removed quite some noise there, but let's *hear* the residuals. I've also placed the wave file [here](../caruso_residuals.wav).
